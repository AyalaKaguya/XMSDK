        ...
        public static void TestCommandTransmission()
        {
            ...
            var server = SocketBuilder.Server("127.0.0.1", 9000)
                .Command("RESET_SYSTEM",
                    (srv, client) => { Console.WriteLine($"[服务器] 收到来自客户端的RESET_SYSTEM命令， 执行重置系统状态"); })
                .Command("UPDATE_CONFIG",
                    (srv, client) => { Console.WriteLine($"[服务器] 收到来自客户端的UPDATE_CONFIG命令， 执行更新配置文件"); })
            ...
            var client3 = SocketBuilder.Client("127.0.0.1", 9000)
                .OnMessage((client, message) => { Console.WriteLine($"[客户端3] 收到消息: {message}"); })
                .Build();
