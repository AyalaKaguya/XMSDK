...
public class TestEventBus
{
    ...
    public void SubscriptionOrder_And_OneTimeSubscription()
    {
        ...
        // 第二个订阅者保持订阅
        bus.Subscribe<string>(_ => { seq.Add("second"); });
    ...
    public void Unsubscribe_ById_And_UnsubscribeAll()
    {
        ...
        var id1 = bus.Subscribe<string>(_ => { received += 1; });
        var id2 = bus.Subscribe<string>(_ => { received += 10; });
    ...
    public void TypeHierarchy_Matching_BaseClassOrInterface()
    {
        ...
        // 订阅基类
        bus.Subscribe<BaseMsg>("DerivedMsg", _ => { baseReceived = true; });
        // 订阅派生类
        bus.Subscribe<DerivedMsg>("DerivedMsg", _ => { derivedReceived = true; });
    ...
    public void HandlerException_DoesNotPrevent_Others()
    {
        ...
        bus.Subscribe<string>(_ => { called.Add("ok"); });
    ...
    public void CustomExceptionHandler_CanRethrowExceptions()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => 
            bus.Publish("test", (_, exception, _) =>
            {
    ...
    public void ExceptionHandler_WithRouteKey()
    {
        ...
        bus.Subscribe<string>("test-route", _ => throw new InvalidOperationException("boom"));

        // 测试带路由键的发布也能正确处理异常
        var count = bus.Publish("test", "test-route", (_, exception, _) =>
        {
            exceptionsCaught.Add(exception);
            return false; // 不重新抛出
        });
