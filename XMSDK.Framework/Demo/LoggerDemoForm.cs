...
namespace XMSDK.Framework.Demo
{
    ...
    {
        private ILogger<LoggerDemoForm> logger;
        ...
        private void SetupLogging()
        {
            ...
            logger = serviceProvider.GetRequiredService<ILogger<LoggerDemoForm>>();

            // 添加欢迎日志
            logger.LogInformation("欢迎使用日志查看器演示程序V2！");
            logger.LogInformation("功能特性:");
            logger.LogInformation("1. 自动滚动开关（默认开启）");
            logger.LogInformation("2. 支持前景色和背景色配置");
            logger.LogInformation("3. 右键菜单中的自动滚动选项");
            logger.LogInformation("4. 性能优化，支持大量日志流畅显示");
            logger.LogInformation("5. 点击按钮测试不同级别的日志");
        }
        ...
        {
            logger.LogDebug("这是调试日志消息");
        }

        private void BtnInfo_Click(object sender, EventArgs e)
        {
            logger.LogInformation("这是信息日志消息");
        }

        private void BtnWarning_Click(object sender, EventArgs e)
        {
            logger.LogWarning("这是警告日志消息");
        }

        private void BtnError_Click(object sender, EventArgs e)
        {
            logger.LogError("这是错误日志消息");
        }

        private void BtnCritical_Click(object sender, EventArgs e)
        {
            logger.LogCritical("这是严重错误日志消息");
        }
        ...
        private void BtnCustomColor_Click(object sender, EventArgs e)
        {
            ...
            logger.LogInformation("已更新前景色配置");
            logger.LogWarning("警告日志现在是紫色");
            logger.LogError("错误日志现在是深红色");
            logger.LogCritical("严重日志现在是洋红色");
        }
        ...
        private void BtnBackgroundColor_Click(object sender, EventArgs e)
        {
            ...
            logger.LogInformation("已更新背景色配置 - 信息日志有浅青色背景");
            logger.LogWarning("警告日志有浅黄色背景");
            logger.LogError("错误日志有浅珊瑚色背景");
            logger.LogCritical("严重日志有红色背景");
        }
            ...
            using (var inputForm = new Form())
            {
                ...
                if (inputForm.ShowDialog() == DialogResult.OK)
                {
                    if (int.TryParse(textBox.Text, out int maxCount) && maxCount > 0)
                    {
                        ...
                        logger.LogInformation($"最大日志条数已设置为: {maxCount}");
                    }
                }
            }
        }
        ...
        private void BtnToggleAutoScroll_Click(object sender, EventArgs e)
        {
            ...
            logger.LogInformation($"自动滚动已{(loggerList.AutoScroll ? "开启" : "关闭")}");
            
            if (!loggerList.AutoScroll)
            {
                logger.LogInformation("现在新日志不会自动滚动到底部");
                logger.LogInformation("您可以手动滚动查看历史日志");
            }
        }
        ...
        private void BtnStressTest_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                logger.LogInformation($"开始性能测试 - {startTime:HH:mm:ss.fff}");
                
                // 生成1000条不同级别的日志
                for (int i = 1; i <= 1000; i++)
                {
                    var logLevel = (LogLevel)(i % 6 + 1); // 循环使用不同级别
                    logger.Log(logLevel, $"性能测试日志 #{i:D4} - 测试ListView优化效果");
                }
                
                var endTime = DateTime.Now;
                var duration = endTime - startTime;
                logger.LogInformation($"性能测试完成 - {endTime:HH:mm:ss.fff} (耗时: {duration.TotalMilliseconds:F0}ms)");
            }
        }
        ...
        private void BtnHighSpeedLog_Click(object sender, EventArgs e)
        {
            if (logTimer.Interval == 1000)
            {
                ...
                logger.LogWarning("已切换到高速日志模式 - 每100ms一条日志");
            }
            else
            {
                // 切换回正常模式
                ...
                logger.LogInformation("已切换回正常日志模式 - 每秒一条日志");
            }
        }
        ...
        private void BtnAutoLog_Click(object sender, EventArgs e)
        {
            if (logTimer.Enabled)
            {
                ...
                logger.LogInformation("自动日志已停止");
            }
            else
            {
                logTimer.Start();
                btnAutoLog.Text = "停止自动日志";
                logger.LogInformation("自动日志已启动");
            }
        }
        ...
        private void LogTimer_Tick(object sender, EventArgs e)
        {
            ...
            logger.Log(logLevel, $"{message} (自动生成 {DateTime.Now:HH:mm:ss})");
        }
